...
class Classic(Mode):
    def __init__ (self, screen):
        ...
        self.arrow_left_fixe = pygame.transform.scale(pygame.image.load("arrow_left_fixe.png"),
                                                      (self.width_arrow, self.width_arrow))
        self.arrow_down_fixe = pygame.transform.scale(pygame.image.load("arrow_down_fixe.png"),
                                                      (self.width_arrow, self.width_arrow))
        self.arrow_up_fixe = pygame.transform.scale(pygame.image.load("arrow_up_fixe.png"),
                                                    (self.width_arrow, self.width_arrow))
        self.arrow_right_fixe = pygame.transform.scale(pygame.image.load("arrow_right_fixe.png"),
                                                       (self.width_arrow, self.width_arrow))
    ...
    def create_arrows (self):
        for i in range(len(self.tiles)):
            for j in range(len(self.tiles[i])):
                if self.tiles[i][j] == 1:
                    self.arrows.append(Arrow_with_time(self.x_arrow + j * self.width_arrow, self.y_arrow_pop,
                                                       pygame.transform.scale(self.img_arrows[j],
                                                                              (self.width_arrow, self.width_arrow)), j,
                                                       self.tiles_delay[i]))
    ...
    def display_score (self, screen, x, y, w, h, text_color=(0, 0, 0), border_color=(0, 0, 0),
...
class Multi_Arrows(Mode):
    ...
    def create_arrows (self):
        for i in range(len(self.tiles)):
            self.arrows.append(Arrow_with_time(self.x_arrow, self.y_arrow_pop,
                                               pygame.transform.scale(self.img_arrows[self.tiles[i]],
                                                                      (self.width_arrow, self.width_arrow)),
                                               self.tiles[i], self.tiles_delay[i]))
    ...
    def play (self, screen):
        ...
        # Afficher le score
        self.display_score(screen, screen.get_width() // 6, screen.get_height() // 8, screen.get_width() // 6,
                           self.width_arrow)
...
class Chrono(Mode):
    def __init__ (self, screen):
        Mode.__init__(self)
        self.width_arrow = screen.get_width() // 12
        self.x_arrow = screen.get_width() // 2 - self.width_arrow
        self.arrow_multi_fixe = pygame.transform.scale(pygame.image.load("multi_arrow_fixe.png"),
                                                       (self.width_arrow, self.width_arrow))
    ...
    def create_arrows (self):
        for i in range(len(self.tiles)):
            self.arrows.append(Arrow(self.x_arrow, 0,
                                     pygame.transform.scale(self.img_arrows[self.tiles[i]],
                                                            (self.width_arrow, self.width_arrow)), self.tiles[i]))

    def display_timer (self, screen, x, y, w, h, text_color=(0, 0, 0), border_color=(0, 0, 0),
                       background_color=(255, 255, 255)):

        font_size = h // 8
        font = pygame.font.Font(FONT_PATH, font_size)
        text_font = font.render("Chrono: {:.3f}".format(time.time() - self.timer + self.malus_time), True, text_color)
        rect_score = text_font.get_rect(center=(w // 2, h // 2))
        surface_score = pygame.Surface((w, h))
        surface_score.fill(background_color)  # Remplir le rectangle avec la couleur blanc
        pygame.draw.rect(surface_score, border_color, surface_score.get_rect(), 4)  # Dessiner le contour noir
    ...
    def play (self, screen):
        screen.fill("purple")
        screen.blit(self.arrow_multi_fixe, (self.x_arrow, 200))
        # Afficher les images de flèches fixe
        i = 0
        for arrow in self.arrows:
            screen.blit(arrow.img, (arrow.x, arrow.y + i * self.width_arrow + 200))
            i += 1
        # Afficher le chrono
        self.display_timer(screen, screen.get_width() // 6, screen.get_height() // 8, screen.get_width() // 6,
                           self.width_arrow)

    def arrow_pressed (self, direction):
        if direction == self.arrows[0].direction:
            self.delta = abs(self.sum_delta - (time.time() - self.timer))
            self.sum_delta += self.delta
            k = self.wich_label(self.delta)
            self.arrows.pop(0)  # or remove
            return k
        else:
            self.malus_time += 0.5
            return 0
    ...
    def wich_label (self, time):
        if time >= 0 and time < 0.25:
            return 5
        elif time >= 0.25 and time < 0.5:
            return 4
        elif time >= 0.5 and time < 0.75:
            return 3
        elif time >= 0.75 and time < 1:
            return 2
        else:
            return 1
